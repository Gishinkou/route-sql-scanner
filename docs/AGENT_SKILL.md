...
扫描器会枚举 MyBatis XML/注解里的静态可见动态 SQL 分支；例如 `<if>` 会生成包含和省略两种 SQL 变体，`<choose>` 会枚举 `when/otherwise` 分支。因此同一个 mapper statement 可能在报告里出现多行。
...
- 如果同一个 mapper statement 出现多行，说明动态 SQL 被枚举成了多个可能变体，不应解释成重复扫描。
